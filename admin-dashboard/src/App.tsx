...
const ALL_NAV = [
  ...
  { to:'/hr',            label:'HR Dashboard',     icon:'👥', roles:['SUPER_ADMIN','ADMIN','MANAGER'] },
  { to:'/security',      label:'Security Center',  icon:'🔒', roles:['SUPER_ADMIN','ADMIN'] },
];
...
const NAV_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      { to:'/hr',       label:'HR Dashboard', icon:'👥', roles:['SUPER_ADMIN','ADMIN','MANAGER'] },
      { to:'/leaves',   label:'Leave',        icon:'🌴', roles:['SUPER_ADMIN','ADMIN','MANAGER'] },
      { to:'/kpi',      label:'KPI Reports',  icon:'📊', roles:['SUPER_ADMIN','ADMIN','MANAGER'] },
      { to:'/leaderboard', label:'Leaderboard', icon:'🏆', roles:[] },
      { to:'/announcements', label:'Announcements', icon:'📢', roles:['SUPER_ADMIN','ADMIN','MANAGER','TEAM_LEADER'] },
    ],
...
function HRDashboardPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [dashRes, leaveRes, userRes] = await Promise.all([
        API.get('/dashboard/stats').catch(() => ({ data: { data: {} } })),
        API.get('/leaves', { params: { status: 'PENDING', limit: 20 } }).catch(() => ({ data: { data: [] } })),
  ...
  const approveLeave = async (id: string, action: 'APPROVED'|'REJECTED') => {
    setSaving(id);
    try {
  ...
  users.forEach(u => { const d = u.department?.name || 'Unassigned'; deptMap[d] = (deptMap[d] || 0) + 1; });
  const deptData = Object.entries(deptMap).map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count);
  const roleMap: Record<string, number> = {};
  users.forEach(u => { const r = u.role?.displayName || u.role?.name || 'Unknown'; roleMap[r] = (roleMap[r] || 0) + 1; });
  const roleData = Object.entries(roleMap).map(([name, count]) => ({ name, count }));

  const BADGE_COLORS = ['#C8102E','#6366F1','#10B981','#F59E0B','#3B82F6','#8B5CF6'];
  const card: React.CSSProperties = { background:'var(--card)', border:'1px solid var(--border)', borderRadius:14, padding:'18px 20px', position:'relative', overflow:'hidden' };

  return (
    <div style={{ padding:24 }}>
      {toast && (
        <div style={{ position:'fixed', top:20, right:20, zIndex:999, background:'#1F2937', color:'#fff', padding:'12px 18px', borderRadius:12, fontSize:13, fontWeight:600, boxShadow:'0 4px 20px rgba(0,0,0,.3)' }}>
          {toast}
        </div>
      )}
      ...
      <div style={{ marginBottom:24 }}>
        ...
        <p style={{ fontSize:13, color:'var(--muted)', margin:'4px 0 0' }}>Human Resources overview · leave approvals · workforce analytics</p>
      </div>

      {loading ? (
        <div style={{ textAlign:'center', padding:80, color:'var(--muted)', fontSize:14 }}>Loading HR data…</div>
      ) : (
        <>
          {/* ── Stat Row ─────────────────────────────────────── */}
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))', gap:14, marginBottom:24 }}>
            {[
              { label:'Total Employees', value:totalEmp,          icon:'👥', color:'#6366F1' },
              { label:'Active',          value:active,            icon:'✅', color:'#10B981' },
              { label:'Inactive / Suspended', value:inactive,     icon:'⚪', color:'#9CA3AF' },
              { label:'Online Now',      value:online,            icon:'🟢', color:'#059669' },
              { label:'Pending Leaves',  value:leaves.length,     icon:'🌴', color:'#F59E0B' },
            ].map(s => (
              <div key={s.label} style={{ ...card }}>
                <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:s.color, borderRadius:'14px 14px 0 0' }} />
                <div style={{ fontSize:26, fontWeight:900, color:s.color, lineHeight:1, marginBottom:4 }}>{s.value}</div>
                <div style={{ fontSize:12, color:'var(--muted)', fontWeight:600 }}>{s.icon} {s.label}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:24 }}>
            ...
            <div style={card}>
              <div style={{ fontWeight:700, fontSize:14, marginBottom:14 }}>🏛️ Department Breakdown</div>
              {deptData.map((d, i) => (
                <div key={d.name} style={{ marginBottom:10 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, marginBottom:4 }}>
                    <span style={{ fontWeight:600 }}>{d.name}</span>
                    <span style={{ color:'var(--muted)' }}>{d.count} employees</span>
                  </div>
                  <div style={{ height:6, borderRadius:3, background:'var(--border)' }}>
                    <div style={{ height:'100%', borderRadius:3, width:`${totalEmp ? (d.count/totalEmp)*100 : 0}%`, background: BADGE_COLORS[i % BADGE_COLORS.length], transition:'width .5s' }} />
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Role Distribution */}
            <div style={card}>
              <div style={{ fontWeight:700, fontSize:14, marginBottom:14 }}>🎭 Role Distribution</div>
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {roleData.map((r, i) => (
                  <div key={r.name} style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                      <div style={{ width:10, height:10, borderRadius:'50%', background: BADGE_COLORS[i % BADGE_COLORS.length], flexShrink:0 }} />
                      <span style={{ fontSize:13 }}>{r.name}</span>
                    </div>
                    <span style={{ fontSize:13, fontWeight:700, color: BADGE_COLORS[i % BADGE_COLORS.length] }}>{r.count}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:16 }}>🌴 Pending Leave Approvals
              {leaves.length > 0 && <span style={{ marginLeft:8, fontSize:11, padding:'2px 8px', borderRadius:99, background:'#FEF3C7', color:'#92400E', fontWeight:700 }}>{leaves.length} pending</span>}
            </div>
                ...
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                  <thead>
                    <tr style={{ borderBottom:'1px solid var(--border)' }}>
                      {['Employee','Type','From','To','Days','Reason','Action'].map(h => (
                        <th key={h} style={{ padding:'8px 12px', textAlign:'left', fontSize:11, fontWeight:700, color:'var(--muted)', textTransform:'uppercase', letterSpacing:.5, whiteSpace:'nowrap' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {leaves.map((l: any) => {
                      const from = new Date(l.startDate), to = new Date(l.endDate);
                      ...
                      return (
                        <tr key={l.id} style={{ borderBottom:'0.5px solid var(--border)' }}>
                          <td style={{ padding:'10px 12px' }}>
                            ...
                            <div style={{ fontSize:11, color:'var(--muted)' }}>{l.user?.employeeId}</div>
                          </td>
                          <td style={{ padding:'10px 12px' }}>
                            <span style={{ padding:'3px 8px', borderRadius:6, fontSize:11, fontWeight:700, background:'#EFF6FF', color:'#1D4ED8' }}>{l.type || l.leaveType}</span>
                          </td>
                          <td style={{ padding:'10px 12px', color:'var(--muted)', whiteSpace:'nowrap' }}>{from.toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'numeric' })}</td>
                          <td style={{ padding:'10px 12px', color:'var(--muted)', whiteSpace:'nowrap' }}>{to.toLocaleDateString('en-GB',   { day:'2-digit', month:'short', year:'numeric' })}</td>
                          <td style={{ padding:'10px 12px', fontWeight:700, color:'#6366F1' }}>{days}d</td>
                          <td style={{ padding:'10px 12px', maxWidth:200 }}>
                            <div style={{ fontSize:12, color:'var(--muted)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{l.reason || '—'}</div>
                          </td>
                          <td style={{ padding:'10px 12px' }}>
                            <div style={{ display:'flex', gap:6 }}>
                              <button disabled={saving === l.id} onClick={() => approveLeave(l.id, 'APPROVED')}
                                style={{ padding:'5px 12px', borderRadius:7, border:'none', background:'#D1FAE5', color:'#065F46', fontWeight:700, cursor:'pointer', fontSize:11, opacity: saving===l.id ? 0.6 : 1 }}>
                                ✅ Approve
                              </button>
                              <button disabled={saving === l.id} onClick={() => approveLeave(l.id, 'REJECTED')}
                                style={{ padding:'5px 12px', borderRadius:7, border:'none', background:'#FEE2E2', color:'#991B1B', fontWeight:700, cursor:'pointer', fontSize:11, opacity: saving===l.id ? 0.6 : 1 }}>
                                ❌ Reject
                              </button>
                            </div>
                          </td>
          ...
          <div style={{ ...card, marginTop:16 }}>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:14 }}>🆕 Employee Directory (Latest)</div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:10 }}>
              {users.slice(0, 12).map((u: any) => (
                <div key={u.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', borderRadius:10, border:'0.5px solid var(--border)', background:'var(--bg)' }}>
                  ...
                  <div style={{ minWidth:0 }}>
                    ...
                    <div style={{ fontSize:11, color:'var(--muted)' }}>{u.role?.displayName || u.role?.name}</div>
                    <div style={{ display:'flex', alignItems:'center', gap:4, marginTop:2 }}>
                      <div style={{ width:6, height:6, borderRadius:'50%', background: u.isOnline ? '#10B981' : '#9CA3AF' }} />
                      <span style={{ fontSize:10, color:'var(--muted)' }}>{u.status}</span>
                    </div>
                  </div>
                </div>
              ))}
...
function SecurityPage() {
  ...
  useEffect(() => {
    Promise.all([
      API.get('/audit',  { params: { limit: 30 } }).catch(() => ({ data: { data: [] } })),
  ...
  const loginLogs = auditLogs.filter(l => l.action?.toLowerCase().includes('login') || l.action?.toLowerCase().includes('auth'));
  const card: React.CSSProperties = { background:'var(--card)', border:'1px solid var(--border)', borderRadius:14, padding:'18px 20px', position:'relative', overflow:'hidden' };

  const ACTION_COLOR: Record<string, string> = {
    CREATE: '#10B981', UPDATE: '#3B82F6', DELETE: '#EF4444',
    LOGIN:  '#6366F1', LOGOUT: '#9CA3AF', DEFAULT: '#F59E0B',
  };
  const getActionColor = (action: string = '') => {
    const k = Object.keys(ACTION_COLOR).find(k => action.toUpperCase().includes(k));
  ...
  return (
    <div style={{ padding:24 }}>
      <div style={{ marginBottom:24 }}>
        <h1 style={{ fontSize:22, fontWeight:700, margin:0 }}>🔒 Security Center</h1>
        <p style={{ fontSize:13, color:'var(--muted)', margin:'4px 0 0' }}>Access control · audit trail · active sessions · role permissions</p>
      </div>

      {loading ? (
        <div style={{ textAlign:'center', padding:80, color:'var(--muted)', fontSize:14 }}>Loading security data…</div>
      ) : (
        <>
          {/* ── Security Stats ───────────────────────────────── */}
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:14, marginBottom:24 }}>
            {[
              { label:'Active Sessions',     value:online,       icon:'🟢', color:'#10B981' },
              { label:'Total Users',         value:users.length, icon:'👥', color:'#6366F1' },
              { label:'Suspended Accounts',  value:suspended,    icon:'🚫', color:'#EF4444' },
              { label:'Roles Defined',       value:roles.length, icon:'🎭', color:'#F59E0B' },
              { label:'Recent Audit Events', value:auditLogs.length, icon:'📋', color:'#3B82F6' },
            ].map(s => (
              <div key={s.label} style={{ ...card }}>
                <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:s.color, borderRadius:'14px 14px 0 0' }} />
                <div style={{ fontSize:26, fontWeight:900, color:s.color, lineHeight:1, marginBottom:4 }}>{s.value}</div>
          ...
          <div style={{ ...card, marginBottom:16 }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
              <div style={{ fontWeight:700, fontSize:14 }}>🔐 Role Permissions Overview</div>
              <NavLink to="/rights" style={{ fontSize:12, color:'var(--primary)', fontWeight:600, textDecoration:'none' }}>
                Manage Rights →
              </NavLink>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:10 }}>
              {roles.map((r: any) => {
                ...
                const ROLE_BG: any = { SUPER_ADMIN:'#FEF2F2', ADMIN:'#FFF7ED', MANAGER:'#FFF0F2', TEAM_LEADER:'#EFF6FF', EMPLOYEE:'#F0FDF4' };
                const ROLE_C:  any = { SUPER_ADMIN:'#EF4444', ADMIN:'#F97316', MANAGER:'#C8102E', TEAM_LEADER:'#3B82F6', EMPLOYEE:'#10B981' };
                return (
                  <div key={r.id} style={{ padding:'12px 14px', borderRadius:10, background: ROLE_BG[r.name] || 'var(--bg)', border:`1px solid ${ROLE_C[r.name] || 'var(--border)'}40` }}>
                    <div style={{ fontWeight:800, fontSize:12, color: ROLE_C[r.name] || 'var(--primary)', marginBottom:4 }}>{r.name.replace(/_/g,' ')}</div>
                    <div style={{ fontSize:11, color:'var(--muted)', marginBottom:6 }}>{r._count?.users ?? 0} users</div>
                    <div style={{ fontSize:12, fontWeight:600 }}>
                      {hasAll ? '⭐ All permissions' : `${perms.length} permissions`}
                    </div>
                  </div>
                );
              })}
          ...
          {online > 0 && (
            <div style={{ ...card, marginBottom:16 }}>
              <div style={{ fontWeight:700, fontSize:14, marginBottom:12 }}>🟢 Currently Online ({online})</div>
              <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                {users.filter(u => u.isOnline).map((u: any) => (
                  <div key={u.id} style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 12px', borderRadius:10, background:'#F0FDF4', border:'1px solid #BBF7D0' }}>
                    <div style={{ width:30, height:30, borderRadius:'50%', background:'#10B981', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:700, fontSize:11 }}>
                      {u.firstName?.[0]}{u.lastName?.[0]}
                    </div>
                    <div>
                      <div style={{ fontWeight:600, fontSize:12 }}>{u.firstName} {u.lastName}</div>
                      <div style={{ fontSize:10, color:'#065F46' }}>{u.role?.displayName || u.role?.name}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div style={card}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
              <div style={{ fontWeight:700, fontSize:14 }}>📋 Recent Audit Trail</div>
              <NavLink to="/audit" style={{ fontSize:12, color:'var(--primary)', fontWeight:600, textDecoration:'none' }}>
                Full Audit Logs →
              </NavLink>
            </div>
            {auditLogs.length === 0 ? (
              <div style={{ textAlign:'center', padding:'20px 0', color:'var(--muted)', fontSize:13 }}>No audit events yet</div>
            ) : (
              <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                {auditLogs.slice(0, 15).map((log: any, i: number) => (
                  <div key={log.id || i} style={{ display:'flex', alignItems:'flex-start', gap:10, padding:'10px 12px', borderRadius:10, background:'var(--bg)', border:'0.5px solid var(--border)' }}>
                    <div style={{ width:8, height:8, borderRadius:'50%', background: getActionColor(log.action), marginTop:4, flexShrink:0 }} />
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
                        <span style={{ fontSize:12, fontWeight:700, color: getActionColor(log.action), padding:'2px 6px', borderRadius:5, background: getActionColor(log.action)+'20' }}>
                          {log.action}
                        </span>
                        <span style={{ fontSize:12, color:'var(--muted)' }}>{log.entity}</span>
                      </div>
