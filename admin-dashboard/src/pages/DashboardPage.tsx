...
function ManagerDashboard() {
  ...
  const filtered = allUsers.filter(u =>
    !search || `${u.firstName} ${u.lastName} ${u.employeeId}`.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div>
      ...
      <div style={{
        margin: '0 20px', marginTop: 20, padding: '14px 16px', borderRadius: 14,
        background: 'linear-gradient(135deg,#EFF6FF,#DBEAFE)',
        border: '1.5px solid #BFDBFE', marginBottom: 4,
      }}>
        <div style={{ fontSize: 11, fontWeight: 800, color: '#3B82F6', letterSpacing: .7,
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
            <input
              ...
              style={{
                ...
                border: '1.5px solid #BFDBFE', background: '#fff', outline: 'none',
                boxSizing: 'border-box', color: '#111827',
              }}
            />
            {showPicker && filtered.length > 0 && (
              <div style={{
                position: 'absolute', top: '110%', left: 0, right: 0, zIndex: 99,
                background: '#fff', border: '1px solid var(--border)', borderRadius: 10,
                boxShadow: '0 8px 32px rgba(0,0,0,.12)', maxHeight: 240, overflowY: 'auto',
              }}>
                {filtered.slice(0, 20).map(u => (
                  <div key={u.id}
                    onClick={() => { setSelectedUser(u); setSearch(`${u.firstName} ${u.lastName}`); setShowPicker(false); }}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 10, color: 'var(--muted)' }}>
                        {u.employeeId} · {u.role?.displayName || u.role?.name} · {u.department?.name || '—'}
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {selectedUser && (
            <button
              ...
              style={{ padding: '10px 16px', borderRadius: 10, border: '1.5px solid #BFDBFE',
                background: '#fff', color: '#3B82F6', fontWeight: 700, cursor: 'pointer', fontSize: 13 }}
            >
              ✕ Clear
            </button>
          )}
        ...
        {selectedUser && (
          <div style={{ marginTop: 8, fontSize: 11, color: '#3B82F6' }}>
            Showing dashboard for: <strong>{selectedUser.firstName} {selectedUser.lastName}</strong> ({selectedUser.employeeId})
          </div>
        )}
      </div>

      {/* Show selected employee's personal dashboard */}
      {selectedUser ? (
        <EmployeeDashboard
        ...
        /* Default: show company-wide overview */
        <AdminDashboard />
      )}
    </div>
  );
